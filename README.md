...
- 表格檢視：結果區可以切換成類似 bc.godfat.org 的表格，底色規則也相同（依格子分數區間、已擁有、限定角色）。每個卡池兩側有箭頭道，路線要抽的格子用抽法顏色的向下箭頭標示；捲出畫面的箭頭道會疊在左右邊緣。卡池可以收合，表格可以拖曳捲動。點格子可以「指定」：必須在那一格抽到那隻角色（位置和角色都要符合），規劃器把它當成必抽條件。點列號代表已經抽到那一列，和路線頁的「已抽」同步。
- 白金券：可以輸入張數並勾選白金轉蛋的必抽角色。每用一張白金券扣 1000 分，所以只會為了確保必抽角色而使用。資料只保留最新 3 期白金轉蛋，角色依所屬系列分組（在 `scripts/build-data.rb` 計算）。
...
```sh
node test/verify-engine.js   # 和 bc.godfat.org 的表格逐格比對（保底卡池 385 格＋白金轉蛋 200 格）
